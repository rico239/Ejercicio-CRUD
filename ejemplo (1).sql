...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 11-05-2018 a las 12:56:45
...
(354, 'Martin', 'INFORMATIC', 7499, '2018-04-27', 1500.00, 300.00, 70),
...
(10000, 'PACO', 'INFORMATIC', 7499, '2018-04-27', 1500.00, 300.00, 70);

-- --------------------------------------------------------

--
-- Estructura Stand-in para la vista `empleadosydepartamentos`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `empleadosydepartamentos` (
`emp_no` smallint(4)
,`apellido` varchar(10)
,`dnombre` varchar(15)
,`loc` varchar(15)
...
--
-- Estructura para la vista `empleadosydepartamentos`
--
DROP TABLE IF EXISTS `empleadosydepartamentos`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `empleadosydepartamentos`  AS  select `empleados`.`emp_no` AS `emp_no`,`empleados`.`apellido` AS `apellido`,`departamentos`.`dnombre` AS `dnombre`,`departamentos`.`loc` AS `loc` from (`empleados` left join `departamentos` on((`empleados`.`dept_no` = `departamentos`.`dept_no`))) ;
